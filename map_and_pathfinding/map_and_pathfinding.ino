...
void loop() {
  ...
  if (n == 0){
    if (h_up >= 0 and stock[h_up][robot_y] == 0){
      stock[h_up][robot_y] = m;
    }
    if (h_down <= 4 and stock[h_down][robot_y] == 0){
      stock[h_down][robot_y] = m;
    }
    if (v_left >= 0 and stock[robot_x][v_left] == 0){
      stock[robot_x][v_left] = m;
    }
    if (v_right <= 4 and stock[robot_x][v_right] == 0){
  ...
  v_left = shelf_y - 1;
  v_right = shelf_y + 1;
  if (v_left >= 0 and v_right <= 4){
    if (stock[shelf_x][v_left] != 0 and stock[shelf_x][v_right] != 0){
  ...
  else if (v_left < 0 and v_right <= 4){
    need_1 = stock[shelf_x][v_right];
    x_1 = shelf_x;
    y_1 = v_right;
}
  ...
  if (h_up >= 0 and h_down <= 4){
    if (stock[h_up][shelf_y] != 0 and stock[h_down][shelf_y] != 0){
...
if (x < shelf_x){
  way[lengtharray - 1] = "вниз";
}
if (x > shelf_x){
  way[lengtharray - 1] = "вверх";
}
if (y < shelf_y){
  way[lengtharray - 1] = "вправо";
}
if (y > shelf_y){
  way[lengtharray - 1] = "влево";  
}
