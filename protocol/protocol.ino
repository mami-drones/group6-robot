...
// Ну сверху понятно, что буфер для чтения пакетов с радио,
// а для чего неедедМессаге нужен и почему он тагой маленький?
// Если ты имел ввиду, что это будет строка с текстовым сообщением 
// для робота, то тебе нужен ```neededMessage *char;```
...
void loop() {
  if (radio.available()) {                         //если есть что принимать, робот работает
     // а если нечего принимать? нихт арбайт?
     // ведь полка не всегда будет слать данные, 
     // да и зачем ей постоянно слать данные, 
     // например, когда робот добирается до полки.
